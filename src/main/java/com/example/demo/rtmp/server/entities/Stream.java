...
import com.example.demo.rtmp.amf.AMF0;
import com.example.demo.rtmp.server.rtmp.Constants;
import com.example.demo.rtmp.server.rtmp.messages.*;
...
//		buf.writeInt(0); // timestamp + timestampExtended
	...
	private void createFileStream() {
		File f = new File(NetConfig.getSaveFlVFilePath() + "/" + streamName.getApp() + "_" + streamName.getName());
		try {
	...
	public synchronized void sendEofToAllSubscriberAndClose() {
		if (NetConfig.isSaveFlvFile() && flvout != null) {
			try {
				flvout.close();
			} catch (IOException e) {
