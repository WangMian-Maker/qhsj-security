...
@Api(tags = "爬行动物记录")
@RestController
@RequestMapping("data/reptilePlan")
public class ReptilePlanController {
    @Autowired
    ...
    public List<Model> getImageForBird(@PathVariable("planName") String planName){
        List<String> allOrders=birdService.findAllOrders("爬行动物");
        List<Model> models=new ArrayList<>();
        for(int i=0;i<allOrders.size();i++){
            //List<BirdPlan> thisOrderBirdPlans=birdPlanService.findByOrder(allOrders.get(i));
            Model model=new Model();
            Color color=new Color();
            color.setColor(birdPlanService.findColor3(allOrders.get(i),"爬行动物"));
            model.setItemStyle(color);
            model.setValue(birdPlanService.findCountInOrder(allOrders.get(i),planName,"爬行动物"));
            model.setName(allOrders.get(i));
            //models.add(model);
            List<Model> models1=new ArrayList<>();
            List<String> allFamilies=birdService.findAllFamiliesByOrder(allOrders.get(i),"爬行动物");
            for(int k=0;k<allFamilies.size();k++){
                Model model1=new Model();
                Color color1=new Color();
                color1.setColor(birdPlanService.findColor2(allFamilies.get(k),"爬行动物"));
                model1.setItemStyle(color1);
                model1.setValue(birdPlanService.findCountInFamily(allFamilies.get(k),planName,"爬行动物"));
                model1.setName(allFamilies.get(k));
                //models1.add(model1);
                List<Model> models2=new ArrayList<>();
                List<String> allChineseName=birdService.findAllChineseNameByFamily(allFamilies.get(k),"爬行动物");
                for(int p=0;p<allChineseName.size();p++){
                    Model model2=new Model();
                    Color color2=new Color();
                    color2.setColor(birdPlanService.findColor1(allChineseName.get(p),"爬行动物"));
    ...
    public Page<BiologyPlan> findPage(@PathVariable("pageNum") int pageNum, @PathVariable("pageSize") int pageSize){
        return birdPlanService.findPage(pageNum,pageSize,"爬行动物");
    }
}
