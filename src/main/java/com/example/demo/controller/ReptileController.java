...
public class ReptileController {
    ...
    public Page<Biology> findPage(@PathVariable("pageNum") int pageNum, @PathVariable("pageSize") int pageSize){
        return birdService.findPage(pageNum,pageSize,"爬行动物");
    }

    @PostMapping("/findAll")
    @ApiOperation(value = "查询所有")
    public List<Biology> findAll(){
        return birdService.findAll("爬行动物");
    }
    ...
    public List<GradeCount> findCount(){
        ...
        tmp.add(birdService.findCountByGradeAndType(1L,"爬行动物"));
        tmp.add(birdService.findCountByGradeAndType(2L,"爬行动物"));
        tmp.add(birdService.findCountByGradeAndType(3L,"爬行动物"));
        tmp.add(birdService.findCountByGradeAndType(4L,"爬行动物"));
        return tmp;
    }
}
